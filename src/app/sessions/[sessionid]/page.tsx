...
export default async function SessionViewerPage({
  ...
  const backToStudyHref = `/studies/${encodeURIComponent(session.plan_id)}`;
  const resumeHref = `${backToStudyHref}${anchor}`;
  const editorHref = `/sessions/${encodeURIComponent(p.sessionid)}/edit`;
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <ConfirmDangerAction
              ...
              action={deleteSessionAction.bind(null, p.sessionid)}
            />
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {session.status !== "complete" ? (
              <form action={markCompleteAction.bind(null, p.sessionid)}>
                <button
                  type="submit"
